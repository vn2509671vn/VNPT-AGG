...
{
    public partial class DanhSachBSCNVDaGiao_ThucTrang : System.Web.UI.Page
    {
        ...
        // Lấy danh sách kế hoạch bsc theo tháng năm và kpi của các nhân viên
        public static DataTable getDetailByTimeAndKPI(int thang, int nam, int kpi_id, int nhanviengiao, int loaimau)
        {
            ...
            string sqlDetailByTimeAndKPI = "select nhanviennhan, kehoach, thuchien ";
        ...
        public static Dictionary<String, String> loadBSCByYear(int thang, int nam, int loaimaubsc)
        {
            ...
            sqlNhomByTime += "and nhom_kpi.id = kpi.nhom_kpi ";
            ...
            int slNhanvien = dsNhanVienByTime.Rows.Count; // Tổng số các nhân viên được giao
            ...
            {
                outputHTML += "<th class='no-sort' colspan='" + (slNhanvien*2) + "'>No item</th>";
            }
            else
            {
                for (int nIndexNV = 0; nIndexNV < dsNhanVienByTime.Rows.Count; nIndexNV++)
                {
                    outputHTML += "<th class='no-sort text-center' colspan='2'>" + dsNhanVienByTime.Rows[nIndexNV]["nhanvien_taikhoan"].ToString() + "</th>";
                }
            }
            outputHTML += "</tr>";

            outputHTML += "<tr>";
            for (int nIndexNV = 0; nIndexNV < slNhanvien; nIndexNV++)
            {
            ...
            {
                outputHTML += "<tr><td colspan='" + (slNhanvien*2 + 4) + "' class='text-center'>No item</td></tr>";
            }
            else
            {
                // Hiển thị các KPO
                for (int nIndexNhom = 0; nIndexNhom < dsNhomByTime.Rows.Count; nIndexNhom++)
                {
                    ...
                    outputHTML += "<td colspan='" + (slNhanvien*2 + 4) + "'><strong>" + ten_nhom + "</strong></td>";
                    ...
                    else
                    {
                        for (int nIndexKPI = 0; nIndexKPI < dsKPIByTimeAndNhom.Rows.Count; nIndexKPI++)
                        {
                            ...
                            outputHTML += "<td class='min-width-300'><strong>" + kpi_ten + "</strong></td>";
        ...
        {
            this.Title = "Số liệu bsc nhân viên đã giao theo PTTB";
            try
            {
