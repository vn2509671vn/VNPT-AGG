...
namespace VNPT_BSC.BSC
{
    public partial class CTV_PTTB : System.Web.UI.Page
    {
        ...
        private string maxNgayCapNhat() {
            ...
            Connection cn = new Connection();
            DataTable tmp = new DataTable();
            string sql = "select format(cast(max(a.timekey) as date), 'dd/MM/yyyy') as thoigian from [OLAP].[OLAP].[dbo].[Fact_VNP_TraTruoc_CCBS] a";
            try
            {
                tmp = cn.XemDL(sql);
            }
        ...
        public static string loadTB(int donvi, int thang, int nam, int kieuhienthi, int nhomctv)
        {
            ...
            else
            {
                if (kieuhienthi == 1)
                {
                    ...
                    for (int i = 0; i < gridData.Rows.Count; i++)
                    {
                        count += Convert.ToInt32(gridData.Rows[i]["sl"].ToString());
                        arrOutput += "<tr>";
                        ...
                        arrOutput += "<td style='text-align: center;border:1px solid !important;'><strong>" + gridData.Rows[i]["sl"].ToString() + "</strong></td>";
                        arrOutput += "</tr>";
                    }
                }
            }
            ...
            else
            {
                ...
                arrOutput += "<td style='border: 1px solid !important'></td>";
                arrOutput += "<td style='border: 1px solid !important'></td>";
                arrOutput += "<td style='text-align: center;border: 1px solid !important;'><strong>Tổng : " + count + "</strong></td>";
            }
            arrOutput += "</tr>";
            arrOutput += "</tfoot>";
